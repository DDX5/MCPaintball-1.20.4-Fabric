...
public class MagentaGrenadeEntity extends ThrownItemEntity
{

    public MagentaGrenadeEntity(EntityType<? extends ThrownItemEntity> entityType, World world)
    {
        super(entityType, world);
    }


    public MagentaGrenadeEntity(LivingEntity livingEntity, World world)
    {
        super(MCPaintballEntities.MAGENTA_GRENADE, livingEntity, world);
    }
        ...
        if(!this.getEntityWorld().isClient())
        {
            ...
            for(PlayerEntity player : Players)
            {
                ...
                if(data.contains("team"))
                {
                    ...
                    if(Team != PaintballTeam.MAGENTA.ordinal())
                    {
                        int[] Cache = levelData.Points;
                        Cache[PaintballTeam.MAGENTA.ordinal()] += 1;
    ...
    {
        return MCPaintballItems.MAGENTA_GRENADE;
    }


    @Override
    protected ItemStack getItem() {
        return new ItemStack(MCPaintballItems.MAGENTA_GRENADE);
    }
}
