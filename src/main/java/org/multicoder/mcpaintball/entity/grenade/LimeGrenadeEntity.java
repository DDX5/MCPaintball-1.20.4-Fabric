...
public class LimeGrenadeEntity extends ThrownItemEntity
{

    public LimeGrenadeEntity(EntityType<? extends ThrownItemEntity> entityType, World world)
    {
        super(entityType, world);
    }


    public LimeGrenadeEntity(LivingEntity livingEntity, World world)
    {
        super(MCPaintballEntities.LIME_GRENADE, livingEntity, world);
    }
        ...
        if(!this.getEntityWorld().isClient())
        {
            ...
            for(PlayerEntity player : Players)
            {
                ...
                if(data.contains("team"))
                {
                    ...
                    if(Team != PaintballTeam.LIME.ordinal())
                    {
                        int[] Cache = levelData.Points;
                        Cache[PaintballTeam.LIME.ordinal()] += 1;
    ...
    {
        return MCPaintballItems.LIME_GRENADE;
    }


    @Override
    protected ItemStack getItem() {
        return new ItemStack(MCPaintballItems.LIME_GRENADE);
    }
}
