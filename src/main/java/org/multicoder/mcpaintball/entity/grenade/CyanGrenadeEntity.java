...
public class CyanGrenadeEntity extends ThrownItemEntity
{

    public CyanGrenadeEntity(EntityType<? extends ThrownItemEntity> entityType, World world)
    {
        super(entityType, world);
    }


    public CyanGrenadeEntity(LivingEntity livingEntity, World world)
    {
        super(MCPaintballEntities.CYAN_GRENADE, livingEntity, world);
    }
        ...
        if(!this.getEntityWorld().isClient())
        {
            ...
            for(PlayerEntity player : Players)
            {
                ...
                if(data.contains("team"))
                {
                    ...
                    if(Team != PaintballTeam.CYAN.ordinal())
                    {
                        int[] Cache = levelData.Points;
                        Cache[PaintballTeam.CYAN.ordinal()] += 1;
    ...
    {
        return MCPaintballItems.CYAN_GRENADE;
    }


    @Override
    protected ItemStack getItem() {
        return new ItemStack(MCPaintballItems.CYAN_GRENADE);
    }
}
