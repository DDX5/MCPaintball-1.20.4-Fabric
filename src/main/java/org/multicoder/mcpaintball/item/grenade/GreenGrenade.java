...
import org.multicoder.mcpaintball.entity.grenade.GreenGrenadeEntity;
import org.multicoder.mcpaintball.world.PaintballMatchData;

public class GreenGrenade extends Item {
    public GreenGrenade() {
        super(new Settings());
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient()) {
            ...
            if (data.IsEnabled) {
                ThrownItemEntity E = new GreenGrenadeEntity(user, world);
